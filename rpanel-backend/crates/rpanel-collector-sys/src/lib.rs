//! rpanel-collector-sys
//! Linux 系统信息采集（/proc, /sys + rustix::fs::statvfs）。
                ...
                tokio::select! {
                    _ = ticker.tick() => {
                        ...
                        let cpu = match read_proc_stat_totals() {
                            Ok(b) => {
                                let usage = if let Some(a) = prev_cpu { cpu_usage_between(&a, &b) } else { 0.0 };
                                prev_cpu = Some(b);
                        ...
                        let snap = MetricsSummary {
                            cpu, memory, disk, net: net_details,
                            ts: now_ms(),
                            cpu_cores: cores,
                        };
                        store.set(snap);
...
// ------- CPU (/proc/stat) -------
#[derive(Clone, Copy)]
struct CpuTotals { user:u64, nice:u64, system:u64, idle:u64, iowait:u64, irq:u64, softirq:u64, steal:u64 }
fn read_proc_stat_totals() -> std::io::Result<CpuTotals> {
    ...
    } else { Err(std::io::Error::new(std::io::ErrorKind::Other, "no cpu line")) }
}
fn cpu_usage_between(a:&CpuTotals, b:&CpuTotals) -> f32 {
    let idle_a = a.idle + a.iowait; let idle_b = b.idle + b.iowait;
    let non_a = a.user + a.nice + a.system + a.irq + a.softirq + a.steal;
    let non_b = b.user + b.nice + b.system + b.irq + b.softirq + b.steal;
    let total_a = idle_a + non_a; let total_b = idle_b + non_b;
...
fn read_meminfo() -> std::io::Result<MemorySummary> {
    ...
    for line in s.lines() {
        if line.starts_with("MemTotal:") { total = parse_kb_line(line); }
        else if line.starts_with("MemAvailable:") { available = parse_kb_line(line); }
    }
...
fn read_speed_opt<P: AsRef<Path>>(p: P) -> Option<u64> {
    let s = fs::read_to_string(p).ok()?; let s = s.trim();
    if s.starts_with('-') { return None; }
    s.parse::<u64>().ok()
}
fn read_u64<P: AsRef<Path>>(p: P) -> std::io::Result<u64> {
    let s = fs::read_to_string(p)?; let s = s.trim();
    if s.starts_with('-') { return Ok(u64::MAX); }
    s.parse::<u64>().map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))
}

fn net_delta_bps(a:&[(String, IfCounters)], b:&[(String, IfCounters)], dt:f64) -> Vec<NetIfStat> {
    ...
    for (name, nb) in b.iter() {
        if let Some(na) = map_a.get(name) {
            ...
            let rx_bps = (rx * 8.0 / dt).max(0.0) as u64;
            let tx_bps = (tx * 8.0 / dt).max(0.0) as u64;
